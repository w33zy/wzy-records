...
/**
 * wzy_Records Sanitization Helper Class
 *
 ...
 */
class wzy_Records_Sanitization_Helper {
	...
	public function __construct( $plugin_name ) {
		...
		add_filter( 'wzy_records_settings_sanitize_text', array( $this, 'sanitize_text_field' ) );
		add_filter( 'wzy_records_settings_sanitize_email', array( $this, 'sanitize_email_field' ) );
		add_filter( 'wzy_records_settings_sanitize_checkbox', array( $this, 'sanitize_checkbox_field' ) );
		add_filter( 'wzy_records_settings_sanitize_url', array( $this, 'sanitize_url_field' ) );
	}
	...
	public function settings_sanitize( $input = array() ) {
		...
		$tab = isset( $referrer['tab'] ) ? $referrer['tab'] : wzy_Records_Settings_Definition::get_default_tab_slug();

		// Tab filter
		$input = apply_filters( 'wzy_records_settings_sanitize_' . $tab, $input );
	...
	private function apply_type_filter( $input, $tab, $key ) {
		...
		return apply_filters( 'wzy_records_settings_sanitize_' . $type, $input[$key], $key );
	}

	private function apply_general_filter( $input, $key ) {

		return apply_filters( 'wzy_records_settings_sanitize', $input[$key], $key );
	}

	// Key specific on change hook
	private function do_settings_on_key_change_hook( $key, $new_value ) {

		$old_plugin_settings = get_option( 'wzy_records_settings' );
		...
		if ( isset($old_plugin_settings[$key]) && $old_plugin_settings[$key] !== $new_value ) {
			...
			do_action( 'wzy_records_settings_on_change_' . $key, $new_value, $old_plugin_settings[$key] );
	...
	private function do_settings_on_change_hook( $new_values, $tab ) {
		...
		if ( $changed ) {
			...
			do_action( 'wzy_records_settings_on_change_' . $tab, $new_values, $old_plugin_settings );
